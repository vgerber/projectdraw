...
std::vector<float> getHeightField(int width, int length, float frequency = 0.3f,
...
int main() {
  ...
  sf::Window window(sf::VideoMode(WIDTH, HEIGHT), "Physics3D",
                    sf::Style::Default, ctxSetting);
  activeWindow = &window;
  initCore();
  ...
  RigidBody rGroundBody(rGroundShape, 0.0f, RigidType::KINEMATIC);
  ...
  srand(time(0));
  collision::CollisionShape tBoxShape(
      collision::generateCube(Size(0.0, 0.0, 0.0, 3.0, 3.0, 5.0)));
  TriggerVolume tTestTrigger(tBoxShape, glm::vec3(0.0, 0.0, 2.5));
  triggerIndicator.translate(tTestTrigger.getPosition());
  tTestTrigger.onCollide = [&tTestTrigger, &triggerIndicator](
                               const CollisionObject *collisionObject) {
    if (const RaycastVehicle *vehicle =
            dynamic_cast<const RaycastVehicle *>(collisionObject)) {
      glm::vec3 newPosition(((float)rand() / RAND_MAX) * 50.0f - 25.0f,
                            ((float)rand() / RAND_MAX) * 10.0f - 5.0f, 2.5);
  ...
  collision::CollisionShape rTerrainShape(
  ...
  while (window.isOpen()) {
    ...
    RaycastHitResult hitResult = physicsWorld.rayCastAll(
        glm::vec3(0.0, 10.0, 2.0), glm::vec3(0.0, -10.0, 2.0));
...
std::vector<float> getHeightField(int width, int height, float frequency,
                                  float amplitude) {
