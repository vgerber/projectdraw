...
DemoVehicle::DemoVehicle(DiscreteWorld &world, Scene &scene) {
#ifdef _WIN32
  vehicleMesh = Mesh("C:/Users/Vincent/Documents/Projects/C++/projectdraw_slim/"
                     "examples/assets/basic_car.fbx");
#elif linux
  vehicleMesh = Mesh("/home/vincent/Development/Cpp/projectdraw_slim/examples/"
                     "assets/basic_car.fbx");
#endif
  chassis = static_cast<Mesh *>(vehicleMesh.getChild("Car"));
  chassis->applyMeshRecenter(
      glm::vec3(0.0f, 0.0f, chassis->getSize().depth * 0.5f));
  scene.addObject(*chassis);
  std::string wheelNames[4] = {"WheelBackL", "WheelBackR", "WheelFrontL",
                               "WheelFrontR"};
  for (int i = 0; i < 4; i++) {
  ...
  btCollisionShape *chassisShapePart =
      collision::generateCube(chassis->getSize());
  chassisShape->addChildShape(transform, chassisShapePart);
  // chassisShape->addChildShape(transform,
  // collision::generateCube(chassis->getSize()));
...
RaycastVehicle *DemoVehicle::getVehicleHandle() { return vehicle; }
