...
int main() {
  ...
  sf::Window window(sf::VideoMode(WIDTH, HEIGHT), "3D Demo", sf::Style::Default,
                    settings);
  ...
  Mesh hudCenterCircle =
      pd::circle(100.0f, 30.0f, glm::vec4(0.7f, 1.0f, 0.7f, 0.4f));
  ...
  camera.translate(glm::vec3(-4.0f, 0.0f, 2.0f));
  {
    ...
    camRotator.vectorRotation(camera.getForward(),
  ...
  rotatingCamera.setForward(glm::vec3(0.0f, 0.0f, 0.0f) -
  ...
  {
    // cylinder.setModel(primitives::generateCylinder(0.5f, 1.0f, 20.0f,
    // glm::vec4(1.0f, 0.0f, 1.0f, 1.0f)));
    quad = pd::quad(2.0f, 1.0f, 1.0f, glm::vec4(0.1f, 1.0f, 0.1f, 1.0f));
    // cylinder.setModel(primitives::generateSphere(10, 15, glm::vec4(1.0f,
    // 0.2f, 0.8f, 1.0f)));
  ...
  {
    cube = Mesh("/home/vincent/Development/Cpp/projectdraw/examples/assets/"
                "basic_car.fbx");
    // cube.translate(glm::vec3(-1.0f, -1.0f, 1.0f));
    wheelBaseTransform =
        (static_cast<Mesh *>(cube.getChild("WheelFrontR")))->getTransform();
    cube.settings.normalVisible = true;
    scene.addObject(cube);
  }
  {
    // sphere.setModel(primitives::generateSphere(10, 15, glm::vec4(1.0f, 0.2f,
    // 0.8f, 1.0f)));
  ...
  textSceneName.rotate(
      Rotator(glm::radians(90.0f), glm::vec3(1.0f, 0.0f, 0.0f)));
  ...
  pointGenerator3D.addTexture(
      Texture(ResourceManager::GetPath("/Assets/smoke.png").c_str()),
      TextureType::Diffuse);
  ...
  while (window.isOpen()) {
    ...
    rotatingCamera.translate(
        glm::vec3(10.0f * cos(millis), 5.0f, 10.0f * sin(millis)));
    rotatingCamera.setForward(glm::vec3(0.0f, 0.0f, 0.0f) -
                              rotatingCamera.getPosition());

    flashLight.direction =
        glm::vec3(0.2f * cos(millis), 0.2f * sin(millis), -1.0f);

    // poleLight.intensity = sin(millis * 10) * 0.5 + 1.0;
    // poleLight2.intensity = sin(millis * 10 + 1.6) * 0.5 + 1.0;

    sunLight.changeDirection(
        glm::vec3(1.0f * cos(millis), 1.0f * sin(millis), -0.7f));

    {
      Drawable *backWheel =
          static_cast<Drawable *>(cube.getChild("WheelFrontR"));
      // backWheel->translate(sin(millis * 1.0f), cos(millis * 1.0f), 0.0f);
      // wheelBaseTransform.print();

      Rotator rotator(glm::radians(-millis * 100.0f),
                      glm::vec3(0.0f, 1.0f, 0.0f));
      rotator.applyRotation(
          Rotator(glm::radians(-45.0f), glm::vec3(0.0f, 0.0f, 1.0f)));
      backWheel->rotate(rotator);
      backWheel->translate(wheelBaseTransform.getTranslation() +
    ...
    }
    {
      ...
      rotator.rotateAxis(glm::radians(millis * 200.0f),
                         glm::vec3(0.0f, 0.0f, 1.0f));

      rotator2.rotateAxis(glm::radians(millis * 100.0f),
                          glm::vec3(0.0f, 0.0f, 1.0f));

      rotator3.rotateAxis(glm::radians(millis * 50.0f),
...
void moveDrawable(Drawable &drawable, Camera &camera, float delta) {
  ...
  if (sf::Keyboard::isKeyPressed(sf::Keyboard::A)) {
    ...
    steerRotator.rotateAxis(glm::radians(0.1f) * delta,
                            glm::vec3(0.0f, 0.0f, 1.0f));
    baseRotator.applyRotation(steerRotator);
    drawable.rotate(baseRotator);
    bankingAngle = -maxBanking; // std::max(bankingAngle - maxBanking * delta *
                                // steering, -maxBanking);
  }
  ...
  if (sf::Keyboard::isKeyPressed(sf::Keyboard::D)) {
    ...
    baseRotator.applyRotation(steerRotator);
    drawable.rotate(baseRotator);
    bankingAngle = maxBanking; // std::min(bankingAngle + maxBanking * delta *
                               // steering, maxBanking);
  }
  ...
  */
  ...
  if (sf::Keyboard::isKeyPressed(sf::Keyboard::Right)) {
    camera.translate(camera.getPosition() +
                     camera.getRight() * delta * camSpeed);
  }
  ...
  if (sf::Keyboard::isKeyPressed(sf::Keyboard::N)) {
    camera.translate(camera.getPosition() +
                     camera.getForward() * delta * camSpeed);
  }
