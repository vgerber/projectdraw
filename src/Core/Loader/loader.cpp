...
GLuint Loader::LoadGeometryShader(const char * vertex_file_path, const char * fragment_file_path, const char * geometry_file_path)
{
	// Create the shaders
	GLuint VertexShaderID = glCreateShader(GL_VERTEX_SHADER);
	...
	std::ifstream GeoShaderStream(geometry_file_path, std::ios::in);
	...
	if (InfoLogLength > 0) {
		...
		glGetShaderInfoLog(GeometryShaderID, InfoLogLength, NULL, &GeoShaderErrorMessage[0]);
