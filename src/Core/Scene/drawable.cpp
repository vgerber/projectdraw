...
void Drawable::removeTexture(Texture &texture) {
  diffuseTextures.erase(std::remove_if(diffuseTextures.begin(),
                                       diffuseTextures.end(),
                                       [&texture](const Texture *tex) {
                                         return &texture == tex;
                                       }),
                        diffuseTextures.end());
  alphaTextures.erase(std::remove_if(alphaTextures.begin(), alphaTextures.end(),
                                     [&texture](const Texture *tex) {
                                       return &texture == tex;
                                     }),
                      alphaTextures.end());
  specularTextures.erase(std::remove_if(specularTextures.begin(),
