...
SpotLight::SpotLight() { setup(); }

void SpotLight::beginShadowMapping() {
  glm::vec3 position = getPosition();
  glm::mat4 lightView = glm::lookAt(
      position, position + glm::normalize(direction), glm::vec3(0.0, 1.0, 0.0));
  depthMap.lightSpaceMatrix =
      glm::perspective(glm::radians(glm::degrees(outerCutOff) * 2.f), 1.0f,
                       0.1f, distance) *
      lightView;
  ...
  glUniformMatrix4fv(
      ...
      GL_FALSE, glm::value_ptr(depthMap.lightSpaceMatrix));
}

void SpotLight::endShadowMapping() { glBindFramebuffer(GL_FRAMEBUFFER, 0); }
...
void SpotLight::setDistance(float distance) { this->distance = distance; }

float SpotLight::getDistance() { return distance; }

DepthMap SpotLight::getDepthMap() { return depthMap; }
...
void SpotLight::dispose() { depthMap.dispose(); }
