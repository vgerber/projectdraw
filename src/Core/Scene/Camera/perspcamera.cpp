...
void PerspectiveCamera::setFieldOfView(float fov) { this->fov = fov; }

float PerspectiveCamera::getFieldOfView() { return fov; }
...
ViewFrustum PerspectiveCamera::getViewFrustum(int splits) {
  ...
  glm::mat4 worldRotation =
      getWorldTransform().getRotation().getRotationMatrix();
  ...
  nearCorners.push_back(nearCenter + worldUp * (sizeNear.height * rectSize) -
                        worldRight * (sizeNear.width * rectSize));
  nearCorners.push_back(nearCenter + worldUp * (sizeNear.height * rectSize) +
                        worldRight * (sizeNear.width * rectSize));
  nearCorners.push_back(nearCenter - worldUp * (sizeNear.height * rectSize) -
                        worldRight * (sizeNear.width * rectSize));
  ...
  while (splits > 0) {
    ...
    splitCorners.push_back(splitCenter +
                           worldUp * (sizeSplit.height * rectSize) -
                           worldRight * (sizeSplit.width * rectSize));
    splitCorners.push_back(splitCenter +
                           worldUp * (sizeSplit.height * rectSize) +
                           worldRight * (sizeSplit.width * rectSize));
    splitCorners.push_back(splitCenter -
...
void PerspectiveCamera::updateProjectionMatrix() {
  if (height != 0.0f) {
    projMatrix =
        glm::perspective(glm::radians(fov), width / height, nearZ, farZ);
  } else {
