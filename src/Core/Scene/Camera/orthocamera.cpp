...
OrthographicCamera::OrthographicCamera(glm::vec3 position, glm::vec3 forward,
                                       glm::vec3 up)
...
ViewFrustum OrthographicCamera::getViewFrustum(int splits) {
  ...
  nearCorners.push_back(nearCenter + getUp() * (sizeNear.height * 0.5f) -
                        getRight() * (sizeNear.width * 0.5f));
  nearCorners.push_back(nearCenter + getUp() * (sizeNear.height * 0.5f) +
                        getRight() * (sizeNear.width * 0.5f));
  nearCorners.push_back(nearCenter - getUp() * (sizeNear.height * 0.5f) -
                        getRight() * (sizeNear.width * 0.5f));
  ...
  while (splits > 0) {
    ...
    splitCorners.push_back(splitCenter + getUp() * (sizeSplit.height * 0.5f) -
                           getRight() * (sizeSplit.width * 0.5f));
    splitCorners.push_back(splitCenter + getUp() * (sizeSplit.height * 0.5f) +
                           getRight() * (sizeSplit.width * 0.5f));
    splitCorners.push_back(splitCenter - getUp() * (sizeSplit.height * 0.5f) -
                           getRight() * (sizeSplit.width * 0.5f));
...
void OrthographicCamera::updateProjectionMatrix() {
  projMatrix = glm::ortho(-0.5f * width, 0.5f * width, -0.5f * height,
                          0.5f * height, nearZ, farZ);
}