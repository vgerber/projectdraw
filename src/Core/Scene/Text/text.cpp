...
Font Text::getFont() { return font; }

Size Text::getSize() { return size; }
...
// GL_DYNAMIC_DRAW); 	glEnableVertexAttribArray(0); 	glVertexAttribPointer(0, 4,
// GL_FLOAT, GL_FALSE, 4 * sizeof(GLfloat), 0); 	glBindBuffer(GL_ARRAY_BUFFER,
...
// 	glUniform3f(glGetUniformLocation(shader.getId(), "textColor"), color.x,
// color.y, color.z); 	glActiveTexture(GL_TEXTURE0); 	glBindVertexArray(VAO);
...
// numberof 1 / 64 pixels) 		x += (ch.Advance >> 6); // Bitshift by 6 to get value
// in pixels(2 ^ 6 = 64)
