...
Font::Font(const char *file, GLuint size) { load(file, size); }

glm::ivec2 Font::getVerticalBounds() { return verticalBounds; }
...
void Font::load(const char *file, GLuint size) {
  ...
  for (GLubyte c = 0; c < 128; c++) {
    ...
    verticalBounds.y =
        std::min((int)face->glyph->bitmap.rows - face->glyph->bitmap_top,
                 verticalBounds.y);
