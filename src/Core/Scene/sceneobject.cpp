...
std::vector<SceneObject *> SceneObject::getChildren() const { return children; }
...
void SceneObject::removeChild(SceneObject *sceneObject) {
  ...
  children.erase(std::remove_if(children.begin(), children.end(),
                                [sceneObject](SceneObject *child) {
                                  return sceneObject == child;
                                }),
                 children.end());
...
std::string SceneObject::getId() const { return id; }

void SceneObject::setId(std::string id) { this->id = id; }

Transform SceneObject::getWorldTransform() const {
  return cachedWorldTransform;
}

void SceneObject::beginEdit() { editMode = true; }
...
void SceneObject::addUpdateListener(void *receiver,
                                    std::function<void()> receiverFunction) {
  if (receiver) {
...
void SceneObject::addUpdateTreeListener(
    void *receiver, std::function<void(SceneObject *)> receiverFunction) {
  if (receiver) {
...
void SceneObject::removeUpdateTreeListener(void *receiver) {
  updateTreeListeners.erase(std::remove_if(updateTreeListeners.begin(),
                                           updateTreeListeners.end(),
                                           [receiver](auto listener) {
                                             return receiver == listener.first;
                                           }),
                            updateTreeListeners.end());
}

bool SceneObject::isEditMode() { return editMode; }
