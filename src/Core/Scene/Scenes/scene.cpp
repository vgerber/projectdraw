...
void Scene::setHUD(HUD &hud) { this->hud = &hud; }
...
void Scene::removeSubScene(AbstractRenderer &renderer) {
  subScenes.erase(std::remove_if(subScenes.begin(), subScenes.end(),
                                 [&renderer](SubScene &subScene) {
                                   return subScene.renderer == &renderer;
                                 }),
                  subScenes.end());
}
...
void Scene::configureSubScene(AbstractRenderer &renderer,
                              SubSceneConfig config) {
  for (size_t i = 0; i < subScenes.size(); i++) {
    if (subScenes[i].renderer == &renderer) {
...
void Scene::setup() { glGenTextures(1, &sceneTexture); }
