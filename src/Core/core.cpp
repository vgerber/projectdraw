...
void initCore() {
	...
	Shader shader_texture_basic;
	shader_texture_basic.layers = {
		{ Loader::GetPath("/Shaders/Texture/texture.vertex").c_str(), ShaderType::VERTEX },
		{ Loader::GetPath("/Shaders/Texture/texture.fragment").c_str(), ShaderType::FRAGMENT }
	};
	shader_texture_basic.load();
	Shaders[SHADER_TEXTURE_BASIC] = shader_texture_basic;
	ResourceManager::storeShader(SHADER_TEXTURE_BASIC, shader_texture_basic);
}
