...
Mesh::Mesh(std::string path) { loadMesh(path); }
...
void Mesh::applyMeshTransformation(Transform transfrom) {
  for (size_t i = 0; i < vertices.size(); i++) {
    vertices[i].Position =
        glm::vec3(transfrom.getMatrix() * glm::vec4(vertices[i].Position, 1.0));
  }
  callUpdate();
}
...
void Mesh::applyMeshRecenter(glm::vec3 centerPoint) {
  Size size = getSize();
  glm::vec3 currentCenter(size.x + 0.5f * size.width,
                          size.y + 0.5f * size.height,
                          size.z + 0.5f * size.depth);
  applyMeshOffset(centerPoint - currentCenter);
}

Size Mesh::getSize() { return size; }

void Mesh::loadMesh(std::string path) {
  Log::write(LogType::Info, std::string("Loading mesh from ") + path, "Mesh");
...
void Mesh::processNode(aiNode *node, const aiScene *scene) {
  ...
  glm::vec3 skew;
  glm::vec4 persp;
  glm::decompose(baseTransformMatrix, scale, rotation, translation, skew,
                 persp);
...
void Mesh::processMesh(aiMesh *mesh, const aiScene *scene) {
  ...
  "texture_diffuse"); textures.insert(textures.begin(), diffuseMaps.begin(),
  diffuseMaps.end()); std::vector<sTexture> specularMaps =
  this->loadMaterialTextures(material, aiTextureType_SPECULAR,
  "texture_specular"); textures.insert(textures.begin(), specularMaps.begin(),
  specularMaps.end());
  }
  */
  this->vertices = vertices;
...
/*
std::vector<sTexture> Model::loadMaterialTextures(aiMaterial * mat,
...
this->directory); texture.type = typeName; texture.path = str;
...
void Mesh::triangle(glm::vec3 p1, glm::vec3 p2, glm::vec3 p3) {
  triangle(Vertex(p1, settings.customColor), Vertex(p2, settings.customColor),
...
void Mesh::rect(glm::vec3 position, int width, int height) {
  rect(Vertex(position, settings.customColor), width, height);
}
...
void Mesh::removeVertex(unsigned int index) {
  ...
  indices.erase(std::remove_if(indices.begin(), indices.end(),
                               [index](const unsigned int &vIndex) {
                                 return index == vIndex;
                               }),
                indices.end());
  callUpdate();
