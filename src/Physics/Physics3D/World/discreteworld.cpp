...
DiscreteWorld::DiscreteWorld() { setup(); }

void DiscreteWorld::addCollisionObject(CollisionObject &collisionObject,
                                       int group, int mask) {
  if (RaycastVehicle *vehicle =
          dynamic_cast<RaycastVehicle *>(&collisionObject)) {
    raycastVehicles.push_back(vehicle);
    static_cast<btDynamicsWorld *>(world)->addRigidBody(
        static_cast<btRigidBody *>(vehicle->getCollisionObjectHandle()), group,
        mask);
    static_cast<btDynamicsWorld *>(world)->addVehicle(
        vehicle->getVehicleObjectHandle());
  } else if (RigidBody *rBody = dynamic_cast<RigidBody *>(&collisionObject)) {
    rigidBodies.push_back(rBody);
    static_cast<btDynamicsWorld *>(world)->addRigidBody(
        static_cast<btRigidBody *>(rBody->getCollisionObjectHandle()), group,
        mask);
...
void DiscreteWorld::update(float delta) {
  static_cast<btDiscreteDynamicsWorld *>(world)->stepSimulation(
      delta, maxSimulationSteps, fixedSimulationStep);
