...
std::vector<CollisionResult> CollisionWorld::getCollisions() {
    ...
    for(int i = 0; i < numMainifolds; i++) {
        ...
        btPersistentManifold * contactManifold = world->getDispatcher()->getManifoldByIndexInternal(i);
        collision.objectA = static_cast<CollisionObject*>(contactManifold->getBody0()->getUserPointer());
        collision.objectB = static_cast<CollisionObject*>(contactManifold->getBody1()->getUserPointer());
        collision.collisionPointsA = std::vector<glm::vec3>(contactManifold->getNumContacts());
        collision.collisionPointsB = std::vector<glm::vec3>(contactManifold->getNumContacts());
...
RaycastHitResult CollisionWorld::rayCastAll(glm::vec3 from, glm::vec3 to, bool approximate) {
    btCollisionWorld::AllHitsRayResultCallback result(toBtVec3(from), toBtVec3(to));
    if(approximate) {
    ...
    if(result.hasHit()) {
        ...
        for(int i = 0; i < result.m_hitFractions.size(); i++) {
            ...
            hitCollisionObject = static_cast<CollisionObject*>(result.m_collisionObjects[i]->getUserPointer());
            hResult.hitLocations.push_back(std::make_pair(hitLocation, hitCollisionObject));
...
RaycastHitResult CollisionWorld::rayCast(glm::vec3 from, glm::vec3 to) {
    btCollisionWorld::ClosestRayResultCallback result(toBtVec3(from), toBtVec3(to));
    ...
    if(result.hasHit()) {
        ...
        CollisionObject * hitCollisionObject = static_cast<CollisionObject*>(result.m_collisionObject->getUserPointer());
        hResult.hitLocations.push_back(std::make_pair(hitLocation, hitCollisionObject));
    }
    return hResult;
}

btCollisionWorld * CollisionWorld::getWorldHandle() {
    return world;
}
...
void CollisionWorld::setup() {
    ...
    world = new btCollisionWorld(collisionDispatcher, broadphase, collisionConfig);
}